        ...
        def show_home():
                        ...
                        # Form Tambah Transaksi
                            with st.form("form_transaksi"):
                                ...
                                akun = st.selectbox("Akun", ["Kas","Persediaan", "Perlengkapan", "Persediaan Kelinci", "Utang Bank", "Utang Gaji", "Modal", "Penjualan", "Beban Pemeliharaan", "Beban Bunga", "Beban Pengiriman",
                                                            "Beban Listrik", "Beban Air", "Beban Gaji", "Ikhtisar Laba Rugi"])
                            ...
                            if not df.empty:
                                for i, row in df.iterrows():
                                    ...
                                    with col1:
                                        st.write(f"Tanggal {row['Tanggal']} | Akun {row['Akun']} | Debit Rp {row['Debit']:,.2f} | Kredit Rp {row['Kredit']:,.2f}")
                                    with col2:
                                        if st.button("🗑️", key=f"hapus_{i}"):
                                            st.session_state['jurnal_umum'] = df.drop(index=i).reset_index(drop=True)
                                            st.rerun()
            ...
            # Pastikan jurnal sudah ada
                ...
                # Kelompokkan akun-akun
                                ...
                                # Hitung Aset
                                aset_df = df[df["Akun"].isin(akun_aset)]
                                total_aset = aset_df.groupby("Akun").agg({"Debit": "sum", "Kredit": "sum"})
                                total_aset["Jumlah"] = total_aset["Debit"] - total_aset["Kredit"]
                                total_aset_nilai = total_aset["Jumlah"].sum() 

                                st.write("### Aset")
                                st.dataframe(total_aset[["Jumlah"]].reset_index())
                                st.write(f"**Total Aset:** Rp {total_aset_nilai:,.2f}")

                                # Hitung Kewajiban
                                total_kewajiban = df[df["Akun"].isin(akun_kewajiban)].groupby("Akun")["Kredit"].sum()
                                st.write("### Kewajiban")
                                st.dataframe(total_kewajiban.reset_index().rename(columns={"Kredit": "Jumlah"}))
                                st.write(f"**Total Kewajiban:** Rp {total_kewajiban.sum():,.2f}")

                                # Ekuitas: modal akhir dari perhitungan sebelumnya
                                st.write("### Ekuitas")
                                ...
                                if abs(total_aset_nilai - total_passiva) < 1:
